...
if (isset($_POST['interface'])) {
    ...
    if (!file_exists($jsonFile)) {
        ...
        exec("python3 " . RASPI_SCRIPTS . "/yaml_operations.py" . " yaml_to_json " . NETPLAN_CONFIG_FILE);
    }
    // Read json file generated by script and located in RASPI_CONFIG_NETWORKING
    $data = file_get_contents($jsonFile);
    $jsonData = json_decode($data, true);
    $jsonData = ['return'=>0,'output'=>$jsonData];
    echo json_encode($jsonData);
}
else {
    $jsonData = ['return'=>1,'output'=>['Error getting data']];
    echo json_encode($jsonData);
}
